...
def run_inference_and_evaluate_3D(
    model, 
    batch, 
    roi_size, 
    plot_images=False, 
    ...
    # Pad the image to match ROI_SIZE
    fun_pad = monai.transforms.SpatialPad(spatial_size=roi_size)
    
    with torch.no_grad():        
        x = batch['INPUT'].to(device)
        y = batch['LABEL'].cpu()

        print('before padding: x shape: {}, y shape: {}; Dim: (C,H,W).'.format(x.shape, y.shape))
        fun_crop = monai.transforms.CenterSpatialCrop([np.min(x.shape[2:])]*3)
        x = fun_crop(img=x[0])[None]
        y = fun_crop(img=y[0])[None]
        # x = fun_pad(img=x[0])[None]
        # y = fun_pad(img=y[0])[None]
        print('before padding: x shape: {}, y shape: {}; Dim: (C,H,W).'.format(x.shape, y.shape))
        
        if rot_degree != 0:
            if direction == 3:
                current_angle=[0,0,math.radians(rot_degree)]
            elif direction == 2:
                current_angle=[0,math.radians(rot_degree),0]
            elif direction == 1:
                current_angle=[math.radians(rot_degree),0,0]
            print('Current angle:{}', current_angle)

            rot_img_trans = monai.transforms.Rotate(
                angle = current_angle,
                mode = 'bilinear', # nearest / bilinear
                padding_mode='reflection',
            )
            x = rot_img_trans(img = x[0])[None]
        ...
        df_eval = pd.DataFrame()
        df_eval['ROT'] = [rot_degree+direction]
        df_eval['ACC'] = acc_list
        df_eval['SEN'] = sen_list
        df_eval['SPE'] = spe_list
        df_eval['IOU'] = iou_list
